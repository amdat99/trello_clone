...
type Props = {
  ...
  todos: object;
  setTaskDragging: React.Dispatch<React.SetStateAction<boolean>>;
  i: number;
  id: string;
  setUrl: (taskId: string) => void;
};
function Task({ todo, setTodos, todos, i, id, setUrl, setTaskDragging }: Props) {
  ...
  return (
    <>
      {todo?.id && todo?.id !== undefined && (
        <Draggable draggableId={todo.id.toString()} index={i} key={todo.id}>
          {(provided, snapshot) => {
            snapshot.isDragging && setTaskDragging(true);
            return (
              <div
                key={i}
